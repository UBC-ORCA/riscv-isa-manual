...
#indices# identified by the instruction. In general, a #CIO# instruction may identify
...
relative to the copy of a given #resident# cache block #stored at the provided cache index#, also known as an #indexed cache block#:

* An _invalidate operation_ deallocates the copy of the #indexed# cache block

* A _clean operation_ performs a write transfer to another cache or to memory if
  the data in the copy of the #indexed# cache block have been modified by a store
  operation

* A _flush operation_ atomically performs a clean operation followed by an
  invalidate operation on the #indexed# cache block

Additional details, including the actual operation performed by a given
#cache-index# management instruction, are described in #<<#Ziciom>>#.
...
#or a CIO instruction that identifies an #indexed# cache block,#
the resulting
...
and any overlap are determined from the #indexed# cache blocks.#
...
#Do we have to check PMP/PMA for underlying physical address of an indexed block
...
These instructions operate on the #indexed cache block resident in the cache at the index#
...
A *cio.clean* instruction performs a clean operation on the #indexed# cache block
specified in _rs1_. The offset operand may
be omitted; otherwise, any expression that computes the offset shall evaluate to
zero. The instruction operates on the set of coherent caches accessed by the
agent executing the instruction.
...
A *cio.flush* instruction performs a flush operation on the #indexed# cache block
specified in _rs1_. The offset operand may
be omitted; otherwise, any expression that computes the offset shall evaluate to
zero. The instruction operates on the set of coherent caches accessed by the
agent executing the instruction.
...
A *cio.inval* instruction performs an invalidate operation on the #indexed# cache block
specified in _rs1_. The offset
operand may be omitted; otherwise, any expression that computes the offset shall
evaluate to zero. The instruction operates on the set of coherent caches
accessed by the agent executing the instruction. Depending on CSR programming,
the instruction may perform a flush operation instead of an invalidate
